...
**Never commit an implementation into `packages/core-*`.** Those files ship as stubs raising
`UnsolvedError` / `UnsolvedException`. An answer that lands there turns an exercise into a
freebie and breaks the `status --assert-stubs` check in CI.

Solve in place while working, then `make extract SLUG=<slug>` to move it to `solutions/` and
...
The user wants the exercise done. Edit the stubs in `packages/core-*` in place, run `make test`,
report. Do **not** run `make extract` — that would undo their work into `solutions/`. Do not
touch `solutions/` at all.
...
- Never leave an implementation committed in `packages/core-*` — those ship as stubs. Use
  `make extract`.
